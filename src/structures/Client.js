...
module.exports = class Client extends AkairoClient {
	...
	updateActivity() {
		if (!process.env.DEV)	{
			post(`https://discordbots.org/api/bots/${this.client.user.id}/stats`, { headers: { Authorization: process.env.DBL_API } }).send({ server_count: this.client.guilds.size })
			post(`https://bots.discord.pw/api/bots/${this.client.user.id}/stats`, { headers: { Authorization: process.env.DBO_API } }).send({ server_count: this.client.guilds.size })
		}
